...
7. Distinguish employment duties and skills (what Mihai does or has done as an employee) from services he offers to clients. A skill or past/current job responsibility (e.g. preparing VAT filings for an employer) is evidence of competence, not an offer to perform that activity for a client. Only state that an activity is offered to clients when the EVIDENCE describes it as a service in a dedicated service/pricing entry for that offering. Never present an employment-duty or skill fact as something available for a client to purchase — this applies in particular to regulated activities such as Umsatzsteuervoranmeldung, Steuerberatung, Jahresabschluss/EÜR, or Anlageberatung, none of which are offered as client services.`;

...
// Pre-existing, out-of-D1E-scope findings (not in the D1C tracked register,
// not named in the D1E authorization). Documented in BACKLOG_PRESERVATION.md.
// Each entry is the exact `var(--color-...)` text as it appears in compiled
// CSS; allowlisting is scoped to that literal property reference only.
export const ALLOWLIST = new Set([
  // 8 source files (Callout, Footer, Card, Link, ProjectShowcase, PostCard,
  // PostHeader, PostMeta): `outline: none; ring: 2px solid; ring-color:
  // var(--color-eucalyptus-500); ring-offset: 2px;` — `ring`/`ring-offset`
  // are not real CSS properties either, so the focus-visible indicator is
  // fully absent, not just uncolored. Real, higher-than-P3 severity, but not
  // part of D1C's tracked register or the D1E authorization — flagged in
  // BACKLOG_PRESERVATION.md for prompt follow-up, not fixed in this phase.
  'var(--color-eucalyptus-500)',
  // src/pages/projects/[slug].astro `.related-card` hover/focus theming:
  // `var(--theme-color, var(--color-eucalyptus-600))` / `...-700)` — the
  // outer var has a real fallback chain via an ancestor-scoped --theme-color,
  // but the inner literal fallback itself is undefined. Same disposition.
  'var(--color-eucalyptus-600)',
  'var(--color-eucalyptus-700)',
]);

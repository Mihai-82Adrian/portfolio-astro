...
import { proveDryRun } from './public-lineage.mjs';
...
async function main() {
  ...
  try {
    ...
    // action (see docs/operations/public-release-lineage-strategy.md) - it is documented to use
    // current local `master` as the synthetic parent, so the local ref is named explicitly rather
    // than relying on public-lineage.mjs's auto-resolution, which now fails closed whenever local
    // refs/heads/master and refs/remotes/origin/master disagree (a real, encountered case in this
    // repository's multi-worktree topology).
    const lineage = await proveDryRun(ROOT, path.join(ROOT, '.artifacts/release'), { publicParent: 'refs/heads/master' });

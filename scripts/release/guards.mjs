...
export function verifyWorkflows(root = ROOT) {
    ...
    invariant(JSON.stringify(workflows) === JSON.stringify(['quality-gates.yml', 'release.yml', 'security-audit.yml']), 'Unexpected workflow topology.');
    ...
    const securityAudit = read(root, '.github/workflows/security-audit.yml');
    invariant(!securityAudit.includes('secrets.'), 'Security audit workflow must use only GITHUB_TOKEN, no repository secrets.');
    invariant(!/pages deploy|wrangler deploy/.test(securityAudit), 'Security audit workflow must not deploy.');
    invariant(!/npm (install|update|audit fix)\b.*--save|npm audit fix/.test(securityAudit), 'Security audit workflow must not modify dependencies.');
    invariant(!/gh (pr|issue) create/.test(securityAudit), 'Security audit workflow must not open a PR or issue automatically.');
    invariant(/^\s*schedule:$/m.test(securityAudit) && securityAudit.includes('workflow_dispatch'), 'Security audit workflow must run on a schedule and be manually dispatchable.');

...
test('ROADMAP.md uses the allowed lifecycle and preserves deferred capabilities', () => {
  ...
  assert.match(roadmap, /#### Phase 3-B1 — Public-Safe Preview and Deployment Control\s+Status: `ACTIVE`/);
  assert.match(roadmap, /#### Phase 3-B2 — Repository Security Baseline\s+Status: `NEXT`/);
  assert.match(roadmap, /#### Phase 3-B3 — Operational Monitoring Automation\s+Status: `PLANNED`/);

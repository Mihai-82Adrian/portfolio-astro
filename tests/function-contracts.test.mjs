...
test('chat: explicit fact intent still works when no OPENAI_API_KEY is configured at all — deterministic routing precedes the provider gate', async () => {
    ...
    assert.equal(body.data.mode, 'fact');
    assert.equal(body.data.answer, facts.contact.en.default);
    assert.ok(!calls.some((c) => c.url.includes('api.openai.com')), 'the provider adapter must never be invoked for a deterministic fact answer');
});
...
async function assertFourAllowedThenBlocked(consentHeaders) {
    ...
    const providerCallsAfterFour = calls.filter((c) => c.url.includes('api.openai.com')).length;
    ...
    const providerCallsAfterFifth = calls.filter((c) => c.url.includes('api.openai.com')).length;
...
test('chat: JD analysis quota (1/24h) is independent of the 4-question chat counter', async () => {
    ...
    const providerCalls = calls.filter((c) => c.url.includes('api.openai.com')).length;
...
test('chat: quota-exceeded rejection preserves the standard envelope, headers and request ID', async () => {
    ...
    const providerCallsBeforeRejection = calls.filter((c) => c.url.includes('api.openai.com')).length;
    ...
    assert.equal(calls.filter((c) => c.url.includes('api.openai.com')).length, providerCallsBeforeRejection);
});

...
- I built a **browser-native XRechnung 3.0 generator**: encodes EN 16931 constraints at the type level, generates UBL 2.1 and CII XML, and exports DIN 5008-style PDFs — zero server-side processing for generation itself. Output conformance is checked separately against the official KoSIT v3.0 validator (Config v3.0.2) as an offline CI gate, not inside the browser (see Part IV)
...
The portfolio is a static Astro 7 site. Adding a full interactive application to it without compromising the page weight of every other page required the Islands Architecture.
